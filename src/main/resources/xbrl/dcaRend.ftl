...
<xbrli:xbrl  
	xmlns:dr="http://www.rgs.mef.gov.it/xbrl/bdap/dca/rend/2016-08-26" 
...
<#list factList as tuple>
	...
	<#list tuple.itemList as item>
		<dr:${item.name} contextRef="${item.context}"<#if item.unit??> unitRef="${item.unit}"</#if><#if item.decimals??> decimals="${item.decimals}"</#if>>${item.value}</dr:${item.name}>
	</#list>
	</dr:${tuple.name}>
